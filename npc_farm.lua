...
local DISTANCIA_BUSCA = 1000 -- Aumento para 1000
local RAIO_DANO = 50 -- Raio de dano ampliado
...
local function voarAteNPC(npc)
    ...
    hrp.Anchored = true -- evitar colisão
    while (hrp.Position - alvoPos).Magnitude > DISTANCIA_ATAQUE and getgenv().AutoFarmLigado do
        hrp.CFrame = CFrame.new(hrp.Position + direction * speed * 0.1)
        task.wait(0.05)
    end
    hrp.Anchored = false
end

-- Função para aplicar dano a NPCs dentro do raio
local function aplicarDanoEmArea()
    for _, model in ipairs(workspace:GetDescendants()) do
        if isNPC(model) then
            local dist = (hrp.Position - model.HumanoidRootPart.Position).Magnitude
            if dist <= RAIO_DANO then
                mirar(model) -- Mira no NPC
                atacar()     -- Aplica o ataque
                soltarSkills() -- Solta as habilidades
            end
        end
    end
end
...
task.spawn(function()
    while getgenv().AutoFarmLigado do
        ...
        if alvo and alvo:FindFirstChild("HumanoidRootPart") then
            while alvo.Humanoid.Health > 0 and getgenv().AutoFarmLigado do
                local dist = (hrp.Position - alvo.HumanoidRootPart.Position).Magnitude
                if dist > DISTANCIA_ATAQUE then
                    voarAteNPC(alvo)
                else
                    aplicarDanoEmArea() -- Aplica dano em área
                    task.wait(DELAY)
                end
            end
        else
            task.wait(0.2) -- espera curta até encontrar novo alvo
        end
    end
end)
